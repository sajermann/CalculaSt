...
describe('Utils/valorTotalCalc', () => {
	it(`should test RJ and Contribuinte`, async () => {
		const result = valorTotalCalc({
			calculaSt: mock,
			fecp: 3,
			ipi: 0,
			st: 10,
	...
	it(`should not Contribuinte`, async () => {
		const result = valorTotalCalc({
			calculaSt: { ...mock, clienteContribuinte: false },
			fecp: 3,
			ipi: 0,
			st: 10,
			total: 100,
