...
import { mustDisabled } from '.';

const DESTINO_MERCADORIA = 'destinoMercadoria';
const TIPO_CALCULO = 'tipoCalculo';
const CLIENTE_CONTRIBUINTE = 'clienteContribuinte';
...
describe('Utils/mustDisabled', () => {
	it(`should test ${DESTINO_MERCADORIA}`, async () => {
		const result = mustDisabled({
			calculaSt: mock,
			dataForVerify: DESTINO_MERCADORIA,
		});
		expect(result).toBe(false);
	});

	it(`should test ${TIPO_CALCULO}`, async () => {
		const result = mustDisabled({
			calculaSt: mock,
			dataForVerify: TIPO_CALCULO,
		});
		expect(result).toBe(true);

		const result2 = mustDisabled({
			calculaSt: {
				...mock,
				destinoMercadoria: { name: 'Consumo', id: '' },
	...
	it(`should test ${CLIENTE_CONTRIBUINTE}`, async () => {
		const result = mustDisabled({
			calculaSt: mock,
			dataForVerify: CLIENTE_CONTRIBUINTE,
		});
		expect(result).toBe(true);
	});

	it(`should test ${SIMPLES_NACIONAL}`, async () => {
		const result = mustDisabled({
			calculaSt: mock,
			dataForVerify: SIMPLES_NACIONAL,
		});
		expect(result).toBe(false);
	});
});
