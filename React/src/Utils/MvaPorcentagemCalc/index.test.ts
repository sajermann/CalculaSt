...
import { mvaPorcentagemCalc } from '.';
...
describe('Utils/mvaPorcentagemCalc', () => {
	it(`should return 0`, async () => {
		const result = mvaPorcentagemCalc({
			ncm: '',
			calculaSt: mock,
			mvaDataBase: mvaDb,
		});
	...
	it(`should test consumo`, async () => {
		const result = mvaPorcentagemCalc({
			ncm: '8544.49.00',
			calculaSt: { ...mock, destinoMercadoria: { name: 'Consumo', id: '' } },
			mvaDataBase: mvaDb,
		});
		expect(result).toBe(0);
	});

	it(`should test revenda`, async () => {
		const result = mvaPorcentagemCalc({
			ncm: '8544.49.00',
			calculaSt: { ...mock },
			mvaDataBase: mvaDb,
	...
	it(`should test mvaDataBase empty`, async () => {
		const result = mvaPorcentagemCalc({
			ncm: '8544.49.00',
			calculaSt: { ...mock },
			mvaDataBase: [],
