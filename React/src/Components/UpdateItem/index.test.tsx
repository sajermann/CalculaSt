...
import * as handleUpdateItem from '~/Utils/HandleUpdateItem';
...
describe('Components/UpdateItem', () => {
	it(`should update and save item`, async () => {
		const spy = vi.fn();
		...
		const spyHandleUpdateItem = vi.spyOn(handleUpdateItem, 'handleUpdateItem');
		...
		const { getAllByText, getByTestId, getByLabelText } = render(
			...
			>
				<UpdateItem itemForEditId="90d4db23-3226-47a1-9f56-f75d357846f3" />
			</QueryClientProvider>,
		);

		const buttonOpen = getByTestId('update-item-button');

		fireEvent.click(buttonOpen);

		await waitFor(async () => {
			await expect(getAllByText('UPDATE_ITEM')[0]).toBeInTheDocument();
		});
		...
		await waitFor(
			async () => {
				await expect(spyHandleUpdateItem).toBeCalled();
			},
			{
				timeout: 4000,
			},
